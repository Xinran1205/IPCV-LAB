...
if __name__ == '__main__': 
    ...
    d = 20
    sphere_center_camera0 = np.array([0, 0, d, 1]).reshape(4, 1)

    # 转换为世界坐标系
    sphere_center_world = np.dot(H0_wc, sphere_center_camera0)
    sphere_center_world = sphere_center_world[:3] / sphere_center_world[3]
    ...
    mesh_list.append(sph_mesh)
    H_list.append(np.array(
        [[1, 0, 0, sphere_center_world[0][0]],
         [0, 1, 0, sphere_center_world[1][0]],
         [0, 0, 1, sphere_center_world[2][0]],
         [0, 0, 0, 1]]
    ))
    RGB_list.append([0., 0.5, 0.5])
